	...
	if( is_array($addon_items) && !empty($addon_items['options']) ) {
		...
		if( is_array($addon_items) && !empty($addon_items['addon_items']) ) {
			...
			foreach( $addon_items['addon_items'] as $k => $val ) {
				if( is_array($val) ) {
					$html .= '<li class="rpress-cart-item"><span class="rpress-cart-item-title">'.$val['addon_item_name'].'</span><span class="addon-item-price cart-item-quantity-wrap"><span class="rpress-cart-item-price qty-class">'.rpress_currency_filter( rpress_format_amount( $val['price'] ) ).'</span></li>';
				}
