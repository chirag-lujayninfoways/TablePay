...
function rpress_ajax_add_to_cart() {
	if ( isset( $_POST['fooditem_id'] ) && isset( $_POST['fooditem_price'] ) ) {
		...
		$to_add 								= array();
		$addons 								= !empty( $_POST['post_data'] ) ? $_POST['post_data'] : [];
		$items 									= '';
			...
			foreach(  $addons as $key => $addon ) {
				...
				if( is_array($item_qty) && !empty($item_qty) ) {
					...
					if( $addon_item_like == 'radio' ) {
						...
						$options['addon_items'][$key]['addon_item_name'] = $addon['name'];
					}
...
function rpress_ajax_update_cart_items() {
	...
	if( isset( $_POST['fooditem_id'] ) ) {
		...
		$get_all_items = !empty( $_POST['post_data'] ) ? $_POST['post_data'] : '';
...
function rpress_ajax_edit_food_item() {
	...
	if( !empty( $fooditem_id ) ) {
		...
		$data = str_replace( '{FoodItemId}', $fooditem_id, $data );
...
function rpress_ajax_get_states_field() {
	...
	if( ! empty( $states ) ) {
		...
		$args = array(
			...
			'id'      => $_POST['field_name'],
			'class'   => $_POST['field_name'] . '  rp-form-control',
			'options' => $states,
...
/**
 * Update delivery options when user proceeds to checkout
 *
 * @since       1.0.2
 * @param       void
...
function rpress_check_new_orders() {
  ...
  if( is_array( $order ) && $order[0]->ID != $last_order ) {
  	...
  	if ( $payment_gateway_label == 'cash-on-delivery' || $payment_gateway_label =='test-payment' ) {
